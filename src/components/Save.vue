<template>
  <div class="save" v-if="valid">
    <div class="flex">
      ...
      <div class="flex">
        <img src="/pen-to-square-solid.svg" alt="edit" v-if="!edit" @click="editName(data)" />
        <img src="/check-solid.svg" alt="save-edit" v-if="edit" @click="saveEdit(data)" />
        <h2>{{ data.method }}</h2>
      </div>
    </div>
    ...
    <div v-if="edit">
      <hr />
      <input type="text" placeholder="New Folder" v-model="folder" />
      <p>Folder</p>
      <div class="folders">
...
.save .flex img {
  ...
  height: 18px;
  margin-right: 5px;
  filter: invert(100%) sepia(100%) saturate(0%) hue-rotate(255deg) brightness(108%) contrast(101%);
  opacity: 0.7;
  cursor: pointer;
  transition: 200ms;
}
