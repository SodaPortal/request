<template>
  <div v-if="valid">
    ...
    <span v-if="!edit">
      <img src="/pen-to-square-solid.svg" alt="editfolder" @click="editFolder" />
      <img src="/delete-left-solid.svg" alt="removefolder" @click="removeFolder" />
    </span>
...
async function removeFolder() {
  ...
  Totify.info('Folder removed successfully. Now you are redirected to "Unknown" folder.');
}
...
img {
  width: 16px;
  height: 16px;
  filter: invert(97%) sepia(0%) saturate(132%) hue-rotate(159deg) brightness(90%) contrast(82%);
  cursor: pointer;
  margin: 0;
  margin-right: 5px;
}
...
img:hover {
  filter: invert(100%) sepia(100%) saturate(0%) hue-rotate(159deg) brightness(102%) contrast(103%);
}
</style>
