...
import { generateQueryParameterTail, nestedToRecord } from "../helpers/objectFormatter";
...
export function generateJavaScriptFetch(request: Request): string {
  ...
  // check method
  if ((request.method === "POST" || request.method === "PUT" || request.method === "PATCH") && !bodyTypeIsNone) {
  ...
  if (request.headers.length > 0) {
    code += `  headers: ${JSON.stringify(nestedToRecord(request.headers), null, 4).slice(0, -1)}  }\n`;
  }
